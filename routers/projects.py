...
async def create_project(
    ...
    if request.master_connection_id:
        conn = supabase_service.get_youtube_connection(request.master_connection_id, user_id)
        if not conn:
...
async def update_project(
    ...
    if master_connection_id:
        # Verify ownership
        conn = supabase_service.get_youtube_connection(master_connection_id, user_id)
        if not conn:
             raise HTTPException(status_code=400, detail="Invalid master_connection_id")
    ...
    if updates:
        supabase_service.update_project(project_id, **updates)
        # Log activity
        supabase_service.log_activity(
            user_id=user_id,
            project_id=project_id,
